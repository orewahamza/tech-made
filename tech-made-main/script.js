...
if (usernameSetupInput) {
    usernameSetupInput.addEventListener('input', (e) => {
        ...
        // Always disable button while typing / re-checking
        confirmUsernameBtn.disabled = true;
        clearTimeout(usernameCheckTimeout);

        if (value.length < 3) {
            usernameSetupStatus.textContent = value.length === 0 ? '' : 'Username must be at least 3 characters';
        ...
        usernameCheckTimeout = setTimeout(async () => {
            try {
                const res = await fetch(`/api/user/check-username/${encodeURIComponent(value)}`);
                ...
                usernameSetupStatus.textContent = '⚠ Could not check availability. Try again.';
                usernameSetupStatus.className = 'username-status taken';
                confirmUsernameBtn.disabled = true;
            }
        }, 600);
...
if (confirmUsernameBtn) {
    confirmUsernameBtn.addEventListener('click', async () => {
        const username = usernameSetupInput.value.trim().toLowerCase();
        if (!username || username.length < 3) return;

        // Double-check: re-verify availability before submitting
        confirmUsernameBtn.disabled = true;
        confirmUsernameBtn.textContent = 'Setting...';

        try {
            // Re-check availability right before saving (race condition guard)
            const checkRes = await fetch(`/api/user/check-username/${encodeURIComponent(username)}`);
            if (!checkRes.ok) throw new Error('Server error during availability check');
            const checkData = await checkRes.json();

            if (!checkData.available) {
                usernameSetupStatus.textContent = '✗ Username was just taken. Try another.';
                usernameSetupStatus.className = 'username-status taken';
                confirmUsernameBtn.disabled = true;
            ...
            if (data.success) {
                ...
                showToast(`Welcome, @${data.username}! Username set successfully.`, 'success', 5000);
                // Update nav/UI
                updateUsernameUI();
                updateCreditsUI();
            } else {
                showToast('Error: ' + data.error, 'error');
                usernameSetupStatus.textContent = '✗ ' + data.error;
                usernameSetupStatus.className = 'username-status taken';
                confirmUsernameBtn.disabled = true;
                confirmUsernameBtn.textContent = 'Confirm Username';
            }
...
if (usernameInput) {
    usernameInput.addEventListener('input', (e) => {
        ...
        // Always disable while re-checking
        setUsernameBtn.disabled = true;
        clearTimeout(usernameCheckTimeout);

        if (value.length < 3) {
            usernameStatus.textContent = value.length === 0 ? '' : 'Username must be at least 3 characters';
...
if (setUsernameBtn) {
    setUsernameBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!checkData.available) {
                usernameStatus.textContent = '✗ Username was just taken. Try another.';
                usernameStatus.className = 'username-status taken';
                setUsernameBtn.disabled = true;
            ...
            if (data.success) {
                ...
                showToast(`@${data.username} set successfully!`, 'success');
                updateUsernameUI();
            } else {
                showToast('Error: ' + data.error, 'error');
